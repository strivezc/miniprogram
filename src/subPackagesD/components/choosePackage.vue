<template>
  <view class="choosePackage">
    ...
    <template v-if="state.filterPackageMergeList.length === 0 && state.inactivatedList.length === 0">
    ...
    <template v-else>
      <view class="item flex-s-c"
            ...
            v-for="item in state.filterPackageMergeList"
      ...
      <template v-if="state.inactivatedList.length > 0">
        <text class="title">未激活套餐</text>
        <view class="item flex-s-c" v-for="item in state.inactivatedList" :key="item.packageId">
          <view class="left flex-c">
          ...
          <view class="right-btn flex-c" @click="activePackage(item.packageId)">激活</view>
        </view>
      </template>
    </template>
  </view>
...
const state = reactive({
  packageMergeId: '',
  inactivatedList:[],
  filterPackageMergeList:[],
})

onLoad((query) => {
  getInactivatedList()
  getFilterPackageMergeList()
  if (query.packageMergeId) {
    state.packageMergeId = query.packageMergeId
  }
})

function getInactivatedList() {
  state.inactivatedList= datebookStore.evaluation.list.filter((item) => !item.activateTime)
}
function getFilterPackageMergeList() {
  ...
  state.filterPackageMergeList=[...packageMergeList, ...activatedList]
}
const activePackage = async (packageId) => {
  try {
    const { resultData } = await DatebookService.activateNeedRemindCheck(packageId)
    uni.showModal({
      title: '提示',
      content: resultData ? resultData : '激活套餐的有效期将从激活日开始计算 !确认要激活此套餐吗？',
      confirmColor: '#0087FF',
      success: function(res) {
        if (res.confirm) {
          ...
          DatebookService.activePackage(params).then(async () => {
            uni.showToast({
              title: '激活成功!',
              icon: 'none'
            })
            const { resultData } = await DatebookService.toChildEvaluationDatebook()
            datebookStore.evaluation.packageMergeList = resultData.packageMergeList
            datebookStore.evaluation.list = resultData.list
            getInactivatedList()
            getFilterPackageMergeList()
          })
        }
...
function selectPackage(item) {
  ...
  uni.navigateBack({
    url: `/subPackagesD/evaluation/step3?packageMergeId=${item.packageMergeId}`
  })
}
</script>
