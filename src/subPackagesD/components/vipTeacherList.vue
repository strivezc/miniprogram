    ...
    <view class="li flex-s-c" @click="next(item.userId,item.teacherType,0)" v-for="(item, index) in state.list" :key="index">
      <view class="left-wrap">
        ...
        <view class="text-wrap">
          ...
          <view class="tip" :class="{'active-tip':item.freeCount>0}">{{item.freeCount>0?`空余${item.freeCount}节`:'今日已约满'}}</view>
        </view>
      </view>
...
function next(userId,teacherType,active) {
  const date=datebookStore.vipClass.params.date
  uni.navigateTo({
    url: `/subPackagesD/vipClass/step3?userId=${userId}&date=${date}&teacherType=${teacherType}&active=${active}`
  })
}
...
const getData=async(refresh)=> {
  try {
    const { resultData, totalCount } = await DatebookService.vipTeacherTime(state.params)
...
<style scoped lang="scss">
  .lotteryRecord {
    height: calc(100vh - 86rpx - env(safe-area-inset-bottom));
