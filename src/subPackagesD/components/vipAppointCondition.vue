  ...
  <view class="appointCondition">
    ...
    <view class="search-wrap" v-show="state.activeTab!==null">
      ...
      <view v-show="state.activeTab===1">
        ...
        <conditionItem :array="weekList" title="预约星期" ref="weekListRef" type="multiple"
                       @paramsToSearch="(val,name)=>paramsToSearch(val,name,'weekList')"></conditionItem>
...
function paramsToSearch(val, name, key) {
  ...
  if (key === 'timePeriods') {
    datebookStore.vipClass.params.time = null
    timeRef.value.reset()
    if (val) {
  ...
  if (['goodAt','teacherSpecial','teachGrade'].includes(key)) { // 多选条件的值数组转字符串
    datebookStore.vipClass.params[key]=val.toString()
  } else{ // 其他直接用选中的值
    datebookStore.vipClass.params[key] = val
  }
}
...
function reset() {
  if (state.activeTab === 0) {
    datebookStore.vipClass.params.showAll = 0 // 筛选条件值重置
    datebookStore.vipClass.params.teacherArea = null // 筛选条件值重置
    areaRef.value.reset()
    showAllRef.value.reset()
  } else if (state.activeTab === 1) {
    timeRef.value.reset()
    timePeriodsRef.value.reset()
    weekListRef.value.reset()
    getOption('06:00', '22:30') // 重置后时间筛选项要恢复默认
    datebookStore.vipClass.params.timePeriods = null // 筛选条件值重置
    datebookStore.vipClass.params.time = null // 筛选条件值重置
    datebookStore.vipClass.params.weekList = [] // 筛选条件值重置
    ...
    sexRef.value.reset()
    datebookStore.vipClass.params.goodAt = null // 筛选条件值重置
    datebookStore.vipClass.params.teacherSpecial = null // 筛选条件值重置
    datebookStore.vipClass.params.teachGrade = null // 筛选条件值重置
    datebookStore.vipClass.params.sex = null // 筛选条件值重置
  }
}
...
<style scoped lang="scss">
  .appointCondition {
    ...
    .search-wrap {
      position: fixed;
      z-index: 1;
      top: 85rpx; /* 筛选条件距离顶部的高度，根据标题栏的高度调整 */
      left: 0;
      right: 0;
