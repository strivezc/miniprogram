<template>
  <view class="step5">
    <view class="title-wrap flex-s-c">
      <view class="title">共
        <text class="yellow">{{state.vipDateTimeList.length}}</text>
        节课，选择课程教材
      </view>
...
const state = reactive({
  lastStepInfo: datebookStore.vipClass.lastStepInfo,
  vipDateTimeList: datebookStore.vipClass.lastStepInfo.vipDateTimeList,
  showReplace: datebookStore.vipClass.params.languageType === 0,
  allowSubstituteTeacher: 0,// 是否同意自动代课
  courseList: [],
  times: 0,
  params:{
    week:datebookStore.vipClass.lastStepInfo.week,
    packageMergeId:datebookStore.vipClass.lastStepInfo.packageMergeId,
  }
})
...
const checkLastBookInfo = async () => {
  try {
    const resultData=state.lastStepInfo
    const hasClass=resultData.bookLessonInfo.hasClass
    const lastLessonInfo=resultData.bookLessonInfo.lastLessonInfo
    const languageType=resultData.bookLessonInfo.languageType
    const studentBookGroupInfo=resultData.bookLessonInfo.studentBookGroupInfo
    if (hasClass === 0) {
      datebookStore.bookList = studentBookGroupInfo
      //需要判断返回的历史教材中的id是否存在于现有教材列表，存在则将历史教材的数据填充到第一个列表中
      if (checkJudgeBook(lastLessonInfo.bookGroupId)) {
        state.courseList[0].bookGroupId = lastLessonInfo.bookGroupId
        state.courseList[0].bookId = lastLessonInfo.bookId
        state.courseList[0].bookName = lastLessonInfo.bookName
        state.courseList[0].lessonId = lastLessonInfo.lessonId
        state.courseList[0].lessonName = `${lastLessonInfo.bookLesson + 1} : ${lastLessonInfo.lesson}`
        await autoFillCourse(lastLessonInfo.lessonId, 0)
      }
    } else {
      if (languageType === 0) {
        //没有上一本教材的信息并且是小语种教材
...
const autoFillCourse = async (lessonId, curIndex) => {
  try {
    const nextNum = state.vipDateTimeList.length - curIndex - 1
...
const submit = async () => {
  ...
  if (nullObj) {
    ...
    try {
      uni.showLoading({})
      const params = {
        allowSubstitute: state.allowSubstituteTeacher,
        datebookInfoList: state.courseList,
        ...state.params
      }
      console.log(params, 'params')
      const { resultData } = await DatebookService.vipDatebookSubmitBeforeCheck(params)
...
const submitInfo = async (hash) => {
  try {
    const { resultData } = await DatebookService.vipDatebookSubmitCheck(hash)
    uni.hideLoading()
    uni.redirectTo({
      url: `/subPackagesD/components/result?times=${state.times}&fromPage=vipClass`
    })
  } catch (e) {
    uni.hideLoading()
    if (e.resultCode === 3) {
...
function resetCourseList() {
  // 数据初始化
  let list = []
  for (let i = 0; i < state.vipDateTimeList.length; i++) {
    list.push({
      bookGroupId: '',
      bookId: '', // 分册id
      bookName: '', //分册名
      lessonId: '',// 课文id
      lessonName: '', // 课文名
      dateTime: state.vipDateTimeList[i].dateTime,
      teacherName: state.vipDateTimeList[i].teacherName,
      teacherId: state.vipDateTimeList[i].teacherId,
      isTemplate: state.vipDateTimeList[i].isTemplate,
      teacherType: state.vipDateTimeList[i].teacherType,
      firstWeek: state.vipDateTimeList[i].firstWeek,
    })
  }
