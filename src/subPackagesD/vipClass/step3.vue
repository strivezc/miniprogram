      ...
      <view class="tea-left-wrap">
        ...
        <view class="tea-text-wrap">
          ...
          <view class="tea-num">
            ...
            {{datebookStore.teacherBaseRespDTO.teacherType}}课时/节
          </view>
        </view>
      </view>
      <image src="@/subPackagesD/images/iv_main_speak.png" class="tea-right"></image>
    </view>
    <view class="tea-sign" v-if="datebookStore.teacherBaseRespDTO.signatureContent">
      签名：{{datebookStore.teacherBaseRespDTO.signatureContent}}
    </view>
    <van-config-provider :theme-vars="state.themeVars">
      <van-tabs swipeable animated :active="state.active" line-width="16" title-active-color="#0087FF"
                title-inactive-color="#333333">
        <van-tab title="老师档案">
          <teacherDetail></teacherDetail>
        </van-tab>
        <van-tab title="VIP约课">
          <vipChangeTime></vipChangeTime>
        </van-tab>
      </van-tabs>
    </van-config-provider>
  </view>
...
import vipChangeTime from '../components/vipChangeTime'
